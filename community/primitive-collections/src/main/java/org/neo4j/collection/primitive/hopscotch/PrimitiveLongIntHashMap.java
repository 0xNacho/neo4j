...
public class PrimitiveLongIntHashMap extends HopScotchHashingLongCollection<int[]> implements PrimitiveLongIntMap
{
    ...
    public PrimitiveLongIntHashMap( HashFunction hashFunction, NumberArrayFactory factory, int initialCapacity,
            Monitor monitor )
    {
        super( hashFunction, factory, 4, NULL, initialCapacity, monitor );
    }
    ...
    @Override
    protected int[] putValue( IntArray array, int index, int absIndex, int[] value )
    {
        int prev = array.get( absIndex+2 );
        array.set( absIndex+2, value[0] );
        transport[0] = prev;
        return transport;
    }
